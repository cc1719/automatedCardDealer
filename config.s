...
;  CONFIG  SOSCSEL = DIG         ; SOSC Power Selection and mode Configuration bits (Digital IO selected)
  ...
  CONFIG  PLLDIV = 2		; Divide 8 MHz clock by 2 for 4 MHz input to PLL
  CONFIG  DEBUG = ON

; CONFIG1H
  CONFIG  CP0 = OFF             ; Code Protect 00800-03FFF (Disabled)
  CONFIG  CPUDIV = OSC1		; 8 MHz clock
...
;  CONFIG  ABW = MM              ; Address Bus Width Select bits (8-bit address bus)
  ...
  CONFIG  MODE = MM		; Mocrocontroller mode

; CONFIG3H
  CONFIG  CCP2MX = DEFAULT        ; CCP2 Mux (RC1)
  CONFIG  ECCPMX = DEFAULT        ; ECCP Mux (Enhanced CCP1/3 [P1B/P1C/P3B/P3C] muxed with RE6/RE5/RE4/RE3)
